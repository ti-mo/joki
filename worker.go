...
// RunWorkers runs PingWorkers for every probe in every probemap
	...
	for {
		...
		for buff.Scan() {
			...
			if result, err := pingParser(buff.Text()); err == nil {
				...
				logErr(fmt.Errorf("%s - %s - %s", pset, pval.name, err.Error()))
			}
		}
		...
		if len(pingresults) > 0 {
			...
			logErr(fmt.Errorf("Worker %s - %s  yielded no results, sleeping for %d minute(s)",
...
func pingParser(text string) (pingresult, error) {
	...
	if len(fields) > 1 && fields[1] == ":" {
		...
		return result, fmt.Errorf("Error parsing FPing output: %s", text)
	}
